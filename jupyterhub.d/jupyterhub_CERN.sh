...
mkdir -p $OUT_FOLDER
tar -cz -f $OUT_FOLDER"/CERNHandlers.tar.gz" -C $TMP_FOLDER"/jupyterhub" CERNHandlers
tar -cz -f $OUT_FOLDER"/CERNSpawner.tar.gz" -C $TMP_FOLDER"/jupyterhub" CERNSpawner
tar -cz -f $OUT_FOLDER"/CERNTemplates.tar.gz" -C $TMP_FOLDER"/jupyterhub" templates
tar -cz -f $OUT_FOLDER"/CERNLogos.tar.gz" -C $TMP_FOLDER"/jupyterhub" logo
/bin/cp $TMP_FOLDER"/manifest/code/templates/jupyterhub/jupyterhub_form.html.erb" $OUT_FOLDER"/jupyterhub_form.html.erb"
/bin/cp $TMP_FOLDER"/jupyterhub/scripts/start_jupyterhub.py" $OUT_FOLDER"/start_jupyterhub.py"
